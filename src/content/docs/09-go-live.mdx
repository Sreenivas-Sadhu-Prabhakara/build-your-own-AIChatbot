...
- **After a reboot,** nothing auto-starts. Bring services up in order: Colima + Postgres →
  embeddings server `:8100` → LiteLLM `:4000` → (8-bit quant) `mlx_lm.server :8001` → Go service
  `:8090` → tunnel. Then refresh the proxy URL if it changed.

That's the whole system — local, grounded, Taglish, and live. See the [Appendix](/build-your-own-AIChatbot/99-appendix/)
